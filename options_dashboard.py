...
def pnl_chart(S, K, prem, n, sigma, r, dte_sim, dte_orig, mult=100):
    """
    Grafico P&L professionale per put scoperta.
    - Curva a scadenza con colore dinamico verde/rosso al break-even
    - Curva valore attuale (T+dte_sim) in arancione tratteggiato
    - Bande ±1σ / ±2σ Expected Move
    - Linee verticali: Spot, Strike, Break-even
    - Tooltip ricco: Spot, P&L scad, P&L oggi, Δ da BE, prob ITM
    """
    N_PTS = 600
    be    = K - prem

    # Range X centrato sul break-even ±35%
    x_lo = be * 0.65
    x_hi = S  * 1.20
    px   = np.linspace(x_lo, x_hi, N_PTS)

    # ── Curva a scadenza ──
    pnl_scad = np.where(px < K, px - K + prem, prem) * n * mult

    # ── Curva oggi (dte_sim giorni rimasti) ──
    T_sim    = max(dte_sim / 365.0, 1 / 365.0)
    pnl_oggi = np.array([
        (prem - prezzo_put(Par(S=s_i, K=K, T=T_sim, r=r, sigma=sigma))) * n * mult
        for s_i in px
    ])

    # ── Expected Move basato su dte_orig (non su dte_sim) ──
    em1 = S * sigma * np.sqrt(dte_orig / 365.0)
    em2 = em1 * 2

    # ── Probabilità ITM per tooltip (N(-d2)) ──
    prob_itm = np.array([
        1 - prob_ok(Par(S=s_i, K=K, T=max(dte_sim/365.0, 1/365.0), r=r, sigma=sigma))
        for s_i in px
    ])

    # ── Spezza la curva scadenza in verde (profitto) e rosso (perdita) ──
    # Trova indice più vicino al break-even
    be_idx  = int(np.argmin(np.abs(px - be)))
    px_prof = np.where(px >= be, px, np.nan)
    px_loss = np.where(px <  be, px, np.nan)
    pnl_prof = np.where(px >= be, pnl_scad, np.nan)
    pnl_loss = np.where(px <  be, pnl_scad, np.nan)

    # Custom data per tooltip
    ...
    # ── Bande Expected Move ──
    fig.add_vrect(
        x0=max(S - em2, x_lo), x1=min(S + em2, x_hi),
        fillcolor='rgba(255,255,255,0.018)', layer='below', line_width=0)
    fig.add_vrect(
        x0=max(S - em1, x_lo), x1=min(S + em1, x_hi),
        fillcolor='rgba(0,194,255,0.045)', layer='below', line_width=0)

    # Etichette bande
    fig.add_annotation(x=S + em1 * 0.98, y=1, yref='paper',
        text='1σ', showarrow=False,
        font=dict(size=9, color='rgba(0,194,255,0.45)'),
        xanchor='right', yanchor='top')
    fig.add_annotation(x=S + em2 * 0.98, y=1, yref='paper',
        text='2σ', showarrow=False,
        font=dict(size=9, color='rgba(255,255,255,0.22)'),
        xanchor='right', yanchor='top')

    # ── Fill zone profit/loss ──
    fig.add_trace(go.Scatter(
        x=px, y=np.where(pnl_scad >= 0, pnl_scad, 0),
        fill='tozeroy', fillcolor='rgba(0,200,83,0.07)',
        line=dict(color='rgba(0,0,0,0)'), showlegend=False, hoverinfo='skip'))
    fig.add_trace(go.Scatter(
        x=px, y=np.where(pnl_scad < 0, pnl_scad, 0),
        fill='tozeroy', fillcolor='rgba(255,61,0,0.07)',
        line=dict(color='rgba(0,0,0,0)'), showlegend=False, hoverinfo='skip'))

    # ── Curva oggi ──
    fig.add_trace(go.Scatter(
        x=px, y=pnl_oggi,
        line=dict(color='rgba(255,181,71,0.70)', width=1.5, dash='dot'),
        name=f'Valore attuale  ({dte_sim}gg)',
        hovertemplate=(
            'Valore attuale: <b>%{y:+,.0f} €</b>'
            '<extra></extra>'
        )
    ))

    # ── Curva a scadenza — zona profitto (verde) ──
    fig.add_trace(go.Scatter(
        x=px_prof, y=pnl_prof,
        line=dict(color='#00C853', width=2.5),
        name='A scadenza — profitto',
        showlegend=True,
        hovertemplate=(
            '<b>Spot %{x:,.2f}</b><br>'
            'P&L scadenza: <b style="color:#00C853">%{y:+,.0f} €</b><br>'
            'Valore attuale: <b>%{customdata[0]:+,.0f} €</b><br>'
            'Distanza BE: <b>%{customdata[1]:+,.2f}</b><br>'
            'Prob. ITM: <b>%{customdata[2]:.1f}%</b>'
            '<extra></extra>'
        ),
        customdata=custom
    ))

    # ── Curva a scadenza — zona perdita (rosso) ──
    fig.add_trace(go.Scatter(
        x=px_loss, y=pnl_loss,
        line=dict(color='#FF3D00', width=2.5),
        name='A scadenza — perdita',
        showlegend=True,
        hovertemplate=(
            '<b>Spot %{x:,.2f}</b><br>'
            'P&L scadenza: <b style="color:#FF3D00">%{y:+,.0f} €</b><br>'
            'Valore attuale: <b>%{customdata[0]:+,.0f} €</b><br>'
            'Distanza BE: <b>%{customdata[1]:+,.2f}</b><br>'
            'Prob. ITM: <b>%{customdata[2]:.1f}%</b>'
            '<extra></extra>'
    ...
    # ── Linee verticali ──
    fig.add_vline(x=K,
        line=dict(color='#FFD600', dash='dash', width=1.2),
        annotation=dict(text=f"Strike  {fmt(K,2)}", font=dict(color='#FFD600', size=10, family='DM Mono'),
                        bgcolor='rgba(255,214,0,0.08)', borderpad=4, yshift=8))
    fig.add_vline(x=S,
        line=dict(color='rgba(255,255,255,0.35)', dash='dot', width=1.2),
        annotation=dict(text=f"Spot  {fmt(S,2)}", font=dict(color='#FFFFFF', size=10, family='DM Mono'),
                        bgcolor='rgba(255,255,255,0.06)', borderpad=4, yshift=8))
    fig.add_vline(x=be,
        line=dict(color='#00E5FF', dash='dash', width=1.2),
        annotation=dict(text=f"Pareggio  {fmt(be,2)}", font=dict(color='#00E5FF', size=10, family='DM Mono'),
                        bgcolor='rgba(0,229,255,0.07)', borderpad=4, yshift=8))

    # ── Linea orizzontale profitto massimo ──
    pmax = prem * n * mult
    ...
    fig.update_layout(
        ...
        height=380,
        xaxis=dict(
            title=dict(text='Prezzo Sottostante', font=dict(size=10, color='#4A5568')),
        ...
        yaxis=dict(
            title=dict(text='P&L (€)', font=dict(size=10, color='#4A5568')),
            gridcolor='rgba(255,255,255,0.03)',
            zerolinecolor='rgba(255,255,255,0.03)',
            tickfont=dict(size=10, color='#4A6080'),
            tickprefix='€ ',
        ...
        legend=dict(
            ...
            borderwidth=1,
            font=dict(size=10, color='#8B9FC0'),
            orientation='h',
...
if STRATEGIA == "put_scoperta":
    ...
    # Trade summary + slider su due colonne
    col_sum, col_sld = st.columns([1, 2], gap="large")

    with col_sum:
        be_val   = K - prem
        pmax_val = prem * n_contratti * 100
        em1_val  = spot * sigma * np.sqrt(dte / 365.0)
        sd_dist  = (spot - K) / em1_val if em1_val > 0 else 0
        st.markdown(f"""
        <div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);
                    border-radius:12px;padding:1.1rem 1.3rem;height:100%">
            <div style="font-family:var(--font-mono);font-size:0.55rem;font-weight:600;
                        letter-spacing:0.18em;text-transform:uppercase;color:var(--text-muted);
                        margin-bottom:0.9rem">Trade Summary</div>
            <table style="width:100%;border-collapse:collapse;font-family:var(--font-mono);font-size:0.72rem">
                <tr><td style="color:#4A6080;padding:3px 0">Strike</td>
                    <td style="color:#FFD600;text-align:right;font-weight:600">{fmt(K,2)}</td></tr>
                <tr><td style="color:#4A6080;padding:3px 0">Premio</td>
                    <td style="color:#00C853;text-align:right;font-weight:600">+{fmt(prem,2)}</td></tr>
                <tr><td style="color:#4A6080;padding:3px 0">Break-even</td>
                    <td style="color:#00E5FF;text-align:right;font-weight:600">{fmt(be_val,2)}</td></tr>
                <tr><td style="color:#4A6080;padding:3px 0">Max Profit</td>
                    <td style="color:#00C853;text-align:right;font-weight:600">+{fmt(pmax_val,0)} €</td></tr>
                <tr><td style="color:#4A6080;padding:3px 0">Distanza BE</td>
                    <td style="color:#E2E8F0;text-align:right">{fmt((spot-be_val)/spot*100,2)}%</td></tr>
                <tr><td style="color:#4A6080;padding:3px 0">Strike in σ</td>
                    <td style="color:#A855F7;text-align:right;font-weight:600">{fmt(sd_dist,2)}σ OTM</td></tr>
                <tr><td style="color:#4A6080;padding:3px 0">DTE</td>
                    <td style="color:#E2E8F0;text-align:right">{dte} gg</td></tr>
            </table>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_sld:
        ...
        dte_graf = st.slider(
            ...
            value=dte_max,
            format="%d gg",
            help="La curva arancione mostra il valore attuale della posizione con i giorni rimasti impostati. "
                 "Abbassando lo slider vedi l'effetto del theta decay: la curva si avvicina progressivamente "
                 "al profilo a scadenza (verde/rosso)."
        )
        days_elapsed = dte_max - dte_graf
        pct_elapsed  = days_elapsed / dte_max * 100 if dte_max > 0 else 0
        st.markdown(f"""
        <div style="display:flex;gap:1.5rem;margin-top:0.5rem">
            <div style="font-family:var(--font-mono);font-size:0.68rem;color:var(--text-muted)">
                <span style="color:#FFB547;font-weight:600">{dte_graf}</span> giorni rimasti
            </div>
            <div style="font-family:var(--font-mono);font-size:0.68rem;color:var(--text-muted)">
                Theta decayato: <span style="color:#00C853;font-weight:600">{fmt(pct_elapsed,1)}%</span>
            </div>
            <div style="font-family:var(--font-mono);font-size:0.68rem;color:var(--text-muted)">
                Guadagno tempo: <span style="color:#00C853;font-weight:600">+{fmt(thday * days_elapsed,0)} €</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    fig_pnl = pnl_chart(spot, K, prem, n_contratti, sigma, r, dte_graf, dte)
    st.plotly_chart(fig_pnl, use_container_width=True, config={
        "displayModeBar": True,
    ...
    em2  = em1 * 2
    st.markdown(f"""
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0.8rem;margin-top:0.2rem;margin-bottom:1.8rem">
        <div style="background:rgba(0,194,255,0.04);border:1px solid rgba(0,194,255,0.10);
                    border-radius:10px;padding:0.75rem 1rem">
            <div style="font-family:var(--font-mono);font-size:0.52rem;color:var(--text-muted);
                        letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.35rem">
                Expected Move ±1&sigma; &nbsp;<span style="color:rgba(0,194,255,0.4)">68%</span></div>
            <div style="font-family:var(--font-body);font-size:1.05rem;font-weight:700;color:var(--accent-cyan)">
                &plusmn;{fmt(em1,2)} <span style="font-size:0.72rem;color:var(--text-secondary);font-weight:400">
                ({fmt(em1/spot*100,1)}%)</span></div>
            <div style="font-family:var(--font-mono);font-size:0.60rem;color:var(--text-muted);margin-top:0.25rem">
                {fmt(spot-em1,2)} &mdash; {fmt(spot+em1,2)}</div>
        </div>
        <div style="background:rgba(255,255,255,0.018);border:1px solid rgba(255,255,255,0.05);
                    border-radius:10px;padding:0.75rem 1rem">
            <div style="font-family:var(--font-mono);font-size:0.52rem;color:var(--text-muted);
                        letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.35rem">
                Expected Move ±2&sigma; &nbsp;<span style="color:rgba(255,255,255,0.2)">95%</span></div>
            <div style="font-family:var(--font-body);font-size:1.05rem;font-weight:700;color:#6B7FA0">
                &plusmn;{fmt(em2,2)} <span style="font-size:0.72rem;color:var(--text-muted);font-weight:400">
                ({fmt(em2/spot*100,1)}%)</span></div>
            <div style="font-family:var(--font-mono);font-size:0.60rem;color:var(--text-muted);margin-top:0.25rem">
                {fmt(spot-em2,2)} &mdash; {fmt(spot+em2,2)}</div>
        </div>
        <div style="background:rgba(168,85,247,0.04);border:1px solid rgba(168,85,247,0.12);
                    border-radius:10px;padding:0.75rem 1rem">
            <div style="font-family:var(--font-mono);font-size:0.52rem;color:var(--text-muted);
                        letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.35rem">
                Strike &mdash; Distanza &sigma;</div>
            <div style="font-family:var(--font-body);font-size:1.05rem;font-weight:700;color:#A855F7">
                {fmt(sd_dist,2)}&sigma; OTM</div>
            <div style="font-family:var(--font-mono);font-size:0.60rem;color:var(--text-muted);margin-top:0.25rem">
                Strike {fmt(K,2)} &middot; BE {fmt(K-prem,2)}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
